...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.doppelganger == False:
                    return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place != hive:
            if len(place.bees) > 0:
                if self.min_range <= i and self.max_range >= i:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            a_bee = random_or_none(self.place.bees)
            if a_bee != None:
                self.eat_bee(a_bee)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while place != None:
            if place.ant != None:
                if place.ant not in self.doubled:
                    place.ant.damage *= 2
                    self.doubled.append(place.ant)
                if isinstance(place.ant, BodyguardAnt) and place.ant.container == True:
                    if place.ant.ant != None and place.ant.ant not in self.doubled:
                        place.ant.ant.damage *= 2
                        self.doubled.append(place.ant.ant)
            place = place.exit
...
def apply_effect(effect, bee, duration):
    ...
    n = duration
    def temp_effect(colony):
        nonlocal n
        if n == 0:
            previous_action(colony)
        elif n > 0:
            effect(previous_action)(colony)
            n -= 1
    bee.action = temp_effect
    # END Problem EC
